...
export async function checkFirebaseJwtForReactivation(
  ...
  try {
    ...
    if (!idToken) {
      return res
        .status(401)
        .json({ status: 1, message: "Missing Firebase ID token" });
    }
    ...
    if (!decodedToken) {
      return res
        .status(401)
        .json({ status: 1, message: "Invalid Firebase ID token" });
    }
    ...
    if (!email) {
      return res
        .status(400)
        .json({ status: 1, message: "Email not found in token" });
    }
    ...
    if (!uid) {
      return res
        .status(400)
        .json({ status: 1, message: "UID not found in token" });
    }
    ...
    if (!decodedToken.sub) {
      return res
        .status(400)
        .json({ status: 1, message: "Subject (sub) not found in token" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ status: 1, message: "Invalid token issuer" });
    }

    if (decodedToken.aud !== process.env.FIREBASE_PROJECT_ID) {
      return res
        .status(400)
        .json({ status: 1, message: "Invalid token audience" });
    }

    // get user from db and check if user is deleted or not. If deleted, return unauthorized.
    ...
    if (err.code === "auth/argument-error") {
      return res
        .status(400)
        .json({ status: 1, message: "Invalid token format" });
    }
    if (err.code === "auth/id-token-expired") {
      return res
        .status(401)
        .json({ status: 1, message: "Firebase ID token expired" });
    }
    if (err.code === "auth/id-token-revoked") {
      return res
        .status(401)
        .json({ status: 1, message: "Firebase ID token revoked" });
    }
